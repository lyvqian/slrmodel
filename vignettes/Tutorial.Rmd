...
The method of least squares is commonly used to find ${\hat{\beta_0}}$ and ${\hat{\beta_1}}$ that make ${\hat{Y}}={\hat{\beta_0}}+{\hat{\beta_1}}X$ (equivalently, $Y_i={\hat{\beta_0}}+{\hat{\beta_1}}X_i+{\hat{\epsilon_i}}$) a best-fitting regression line for X and Y. For more details details regarding this method, please refer to the "Least Squares Method" section of [Simple Linear Regression Document](https://rpubs.com/karflisi/slr)) 

slrmodel aims to calculate the ${\hat{\beta_0}}$ and ${\hat{\beta_1}}$ using the method of least squares for a set of (X,Y), test the significance of (X,Y) association, and evaluate the overall fitness of the regression model $Y_i={\hat{\beta_0}}+{\hat{\beta_1}}X_i+{\epsilon_i}$.
...
`slr_beta()` allows you to generate a matrix with 2 rows and 4 columns containing ${\hat{\beta_0}}$, ${\hat{\beta_1}}$, the standard error of the two estimates, the t statistic for testing whether the estimates significantly differ from 0, and the p value for making the statistical decision. 
...
```
The "Estimate" of beta0 gives the value of ${\hat{\beta_0}}$, which is the intercept of the linear model. It tells us the mean value of Y when X=0. Sometimes it may have an awkward interpretation. For example, in this case, we would never see petals with width=0. The "Estimate" of beta1 gives the value of ${\hat{\beta_1}}$, which is the slope of the linear regression model. It describes the mean change in Y per unit change in X. In this example, it is estimated that petal length would increase by 2.23 cm for 1 cm increase in the petal width.
...
Inverting the order of arguments would change the result, so you must not mix up your predictor and response variables when implementing the function:
```{r}
slr_beta(iris$Petal.Length,iris$Petal.Width)
```

### Calculate the coefficient of determination with `slr_rsquared()`
`slr_rsquared()` provides the coefficient of determination R^2^, which is the proportion of the variation in Y explained by the linear regression model. 
...
This section examines correctness and efficiency of the slrmodel functions.

`lm()` function is used to fit linear models, including simple linear regression models and multiple linear regression models. `lm(y~x)` model the response variable `y` as a linear function of the predictor variable `x`. 
...
`slr_beta()` was written by R code, and `slr_rsquared()` was written by C++ code. slrmodel functions is like a simplified version of `lm()` and only generates selected results of `lm()`. In general, slrmodel functions seem to be very efficient compared to `lm()`:
```{r}
if(!require(bench)) install.packages("bench",repos = "http://cran.us.r-project.org")
bench::mark(
  slr_beta_result=matrix(slr_beta(iris$Petal.Width,iris$Petal.Length),nrow=2,ncol=4,dimnames=NULL),
  lm_beta_result=matrix(summary(lm(Petal.Length~Petal.Width,data=iris))$coef,nrow=2,ncol=4,dimnames=NULL) 
)

bench::mark(
  slr_rsquared_result=slr_rsquared(iris$Petal.Width,iris$Petal.Length),
  lm_rsquared_result=summary(lm(Petal.Length~Petal.Width,data=iris))$r.squared
)
```
